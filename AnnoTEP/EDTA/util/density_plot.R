...
# str(repeats)
# sum(is.na(repeats$type))  # Verifique quantos NAs existem na coluna `type`
# sum(is.na(repeats$density))
# sum(is.na(repeats$chrom))
# sum(is.na(repeats$coord))
...
repeats <- repeats %>%
    ...
    #ADDED
    mutate(type = gsub("MITE", "TIR/MITE", type)) %>%
    mutate(type = gsub("TRIM", "LTR/TRIM", type)) %>%
    mutate(type = gsub("BARE-2", "LTR/BARE-2", type)) %>%
    mutate(type = gsub("TR_GAG", "LTR/TR_GAG", type)) %>%
    mutate(type = gsub("LARD", "LTR/LARD", type))
...
fixed_colors <- c(
  ...
  "LTR/LARD" = "#FF5500",
  "LTR/unknown" = "#FF8000",
  "nonLTR/unknown" = "#FFAA00",
  "LINE/unknown" = "#FFD400",
  "LINE/L1" = "#FFFF00",
  "LINE/RTE" = "#D4FF2B",
  "SINE/unknown" = "#AAFF55",
  "TIR/PIF_Harbinger" = "#80FF80",
  "TIR/Mutator" = "#55FFAA",
  "TIR/MITE" = "#2BFFD4",
  "TIR/hAT" = "#00FFFF",
  "TIR/CACTA" = "#00D4FF",
  "TIR/Tc1_Mariner" = "#00AAFF",
  "Helitron" = "#0080FF",
  "rDNA intergenic spacer" = "#CFCFCF",
  "Knob" = "#ADAAAB",
  "Subtelomere" = "#A09D9E",
  "Satellite" = "#989596",
  "repeat_fragment" = "#8C898A",
  "rRNA_gene" = "#807C7D",
  "low_complexity" = "#5B5859"
)
...
repeats$type <- factor(repeats$type, levels = names(fixed_colors))
...
# EDIT / ADDED 
valid_chrom_levels <- chrom_levels[grepl("^[0-9]+$", gsub("[^0-9]", "", chrom_levels))]

chrom_levels_sorted <- valid_chrom_levels[order(as.numeric(gsub("[^0-9]", "", valid_chrom_levels)))]
...
if (merge_plots) {
  p <- ggplot(repeats, aes(x = coord / 1e6, y = density * 100, color = type)) + 
       geom_line() +
       geom_point(size = 0, show.legend = TRUE) +  # Invisible points for legend entry.
       ...
       theme_minimal() +
       plot_border_theme +
       scale_color_manual(values = fixed_colors, guide = guide_legend(override.aes = list(shape = 15, size = 6))) + 
