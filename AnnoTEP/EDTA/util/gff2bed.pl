...
while (<GFF>){
	...
	if ($class =~ /(non_LTR|LINE|LINE_element|LINE_retrotransposon|RIL)/i){
	...
	if ($class =~ /(nonLTR|non_LTR|YR_retrotransposon|Penelope|ERTBV|pararetrovirus|Ngaro|DIRS|Viper)/i){
